...
function deploy_tt_mysql_all_in_one {
  ...
  helm install $tsMysqlName --set mysql.mysqlUser=$tsUser --set mysql.mysqlPassword=$tsPassword --set persistence.storageClass=openebs-hostpath --set mysql.mysqlDatabase=$tsDB $mysqlCharts -n $namespace 1>/dev/null
...
function deploy_tt_mysql_each_service {
  ...
  do
    mysqlName="ts-$s"
    helm install $mysqlName --set mysql.mysqlUser=$tsUser --set mysql.mysqlPassword=$tsPassword --set persistence.storageClass=openebs-hostpath --set mysql.mysqlDatabase=$tsDB $mysqlCharts -n $namespace 1>/dev/null
  done
